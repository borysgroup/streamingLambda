...
1. **`token.pickle`** — the workflow passes the secret as `GOOGLE_OAUTH_PICKLE_TOKEN_B64`,
   but the secret was created as `GOOGLE_OAUTH_TOKEN_PICKLE_B64` (TOKEN/PICKLE swapped), so
   the value reaches the agent env **empty**. Once the names match, run
   [`scripts/token_pickle_roundtrip.py`](../scripts/token_pickle_roundtrip.py): it decodes the
   base64 secret, overwrites the placeholder in S3, then runs the Lambda's own
   download → refresh → re-upload cycle (PR 2, comment No. 46). Everything except the refresh
   itself is already verified against live AWS.
2. ~~**`LAMBDA_FUNCTION_URL` secret**~~ — done (2026-07-08, third session): it holds the bare
   function URL, and both the invalid-action (`400`) and `create` (placeholder `500`) probes
   pass through it. The `picam-http-api` HTTP API is now redundant and can be deleted.
3. ~~**Region env var**~~ — done: `AWS_REGION=us-east-2` reaches the agent env.
